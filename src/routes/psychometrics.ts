...
export function createPsychometricsRouter(
  ...
  router.get("/score-validity", async (_req, res) => {
    try {
      // Summary stats as proxy for validity evidence
      const [sessionCount, reportCount, avgScore] = await Promise.all([
        prisma.session.count({ where: { status: "COMPLETED" } }),
        prisma.scoreReport.count(),
        prisma.scoreReport.aggregate({ _avg: { overallScore: true } }),
      ]);
      res.json({
        sessionCount,
        reportCount,
        averageScore: avgScore._avg.overallScore,
        validityEvidenceVersion: "1.0",
        note: "Full validity evidence package requires concurrent validity data.",
      });
    } catch (err: any) {
