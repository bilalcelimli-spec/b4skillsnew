...
export const AdminDashboard: React.FC = () => {
  const [activeTab, setActiveTab] = useState<"overview" | "candidates" | "branding" | "import" | "integrations" | "audit" | "analytics" | "settings" | "proctoring" | "billing" | "calibration">("overview");
  ...
  useEffect(() => {
    const loadMockSessions = () => {
      const mockSessions: SessionData[] = [
        { id: "demo-sess-1", candidateId: "user-1", organizationId: "b4skills-demo", status: "completed", currentStage: 20, abilityEstimate: 1.2, cefrLevel: "B2", startedAt: new Date(Date.now() - 3600000), completedAt: new Date(), candidateName: "Alice Johnson", candidateEmail: "alice@example.com" },
        { id: "demo-sess-2", candidateId: "user-2", organizationId: "b4skills-demo", status: "in_progress", currentStage: 8, abilityEstimate: 0.3, cefrLevel: "B1", startedAt: new Date(Date.now() - 900000), candidateName: "Ben Carter", candidateEmail: "ben@example.com" },
        { id: "demo-sess-3", candidateId: "user-3", organizationId: "b4skills-demo", status: "completed", currentStage: 20, abilityEstimate: 2.1, cefrLevel: "C1", startedAt: new Date(Date.now() - 7200000), completedAt: new Date(Date.now() - 3600000), candidateName: "Clara Ricci", candidateEmail: "clara@example.com" },
      ];
  ...
  return (
    <div className="space-y-8">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-black text-slate-900 uppercase tracking-tighter">Admin Command Center</h1>
          <p className="text-slate-500 font-medium text-sm">Enterprise Management & Global Analytics</p>
        </div>
        <div className="flex bg-slate-100 p-1 rounded-2xl border border-slate-200">
          ...
          <TabButton 
            ...
            onClick={() => { setActiveTab("branding"); setSelectedSessionId(null); }} 
            icon={<Palette size={14} />} 
            label="Branding" 
          />
          <TabButton 
            active={activeTab === "import"} 
            onClick={() => { setActiveTab("import"); setSelectedSessionId(null); }} 
            icon={<Upload size={14} />} 
            label="Import" 
          />
          <TabButton 
            active={activeTab === "integrations"} 
            onClick={() => { setActiveTab("integrations"); setSelectedSessionId(null); }} 
            icon={<Zap size={14} />} 
            label="Integrations" 
          />
          <TabButton 
          ...
          <TabButton 
            ...
            onClick={() => { setActiveTab("proctoring"); setSelectedSessionId(null); }} 
            icon={<ShieldAlert size={14} />} 
            label="Proctoring" 
          />
          <TabButton 
            active={activeTab === "billing"} 
            onClick={() => { setActiveTab("billing"); setSelectedSessionId(null); }} 
            icon={<CreditCard size={14} />} 
            label="Billing" 
          />
          <TabButton 
            active={activeTab === "analytics"} 
            onClick={() => { setActiveTab("analytics"); setSelectedSessionId(null); }} 
            icon={<BarChart3 size={14} />} 
            label="Analytics" 
          />
          <TabButton 
            active={activeTab === "calibration"} 
            onClick={() => { setActiveTab("calibration"); setSelectedSessionId(null); }} 
            icon={<Calculator size={14} />} 
            label="Calibration" 
          />
          <TabButton 
            active={activeTab === "settings"} 
            onClick={() => { setActiveTab("settings"); setSelectedSessionId(null); }} 
            ...
            {selectedSessionId ? (
              ...
              <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
                ...
                <CardHeader className="flex flex-row items-center justify-between bg-slate-50/50 border-b border-slate-100">
                  <div className="font-black text-slate-900 uppercase tracking-tight text-sm">Recent Assessment Sessions</div>
                  <div className="flex gap-2">
                    <Button variant="outline" size="sm" className="h-8 text-[10px] font-black uppercase tracking-widest rounded-xl">
                    ...
                    <table className="w-full text-left border-collapse">
                      <thead>
                        <tr className="bg-slate-50/30 text-slate-400 text-[8px] uppercase tracking-widest font-black">
                          <th className="px-6 py-4 border-b border-slate-100">Candidate</th>
                          <th className="px-6 py-4 border-b border-slate-100">Status</th>
                          <th className="px-6 py-4 border-b border-slate-100 text-center">Ability (θ)</th>
              ...
              <div className="space-y-6">
                ...
                <Card className="rounded-2xl shadow-sm border-slate-200">
                  <CardHeader className="font-black text-slate-900 uppercase tracking-tight text-sm">Distribution</CardHeader>
                  <CardContent>
  ...
  return (
    <Card className="border-slate-200 hover:border-indigo-200 transition-all">
      <CardContent className="p-6">
        ...
        <div className="mt-4 text-[10px] text-slate-400 font-medium">{trend}</div>
      </CardContent>
    </Card>
  );
}
