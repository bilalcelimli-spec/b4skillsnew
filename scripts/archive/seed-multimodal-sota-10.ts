...
async function generateBatchWithRetry(product: any, batchSize: number, retries = 5): Promise<any[]> {
  const prompt = `
...
EXAMPLE ITEM SCHEMA IN ARRAY is below. (Do not copy this exactly, just use as structure reference):
[
  {
  ...
  },
  {
    ...
    "guessing": 0.25,
    "content": {
      "prompt": "She ____ to the store yesterday.",
      "options": [{"id":"A","text":"go"},{"id":"B","text":"went"},{"id":"C","text":"goes"},{"id":"D","text":"going"}],
      "correctAnswer": "B",
      "rubric": "Irregular past tense testing."
    }
  }
]
`;
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      if (attempt === retries) {
        console.error(`Giving up on this batch after ${retries} failed attempts.`);
        return [];
      }
      // Wait significantly longer before retrying to prevent 503 errors (exponential backoff)
...
async function main() {
  console.log("Starting generation of MULTIMODAL SOTA assessment items in batches of 10...");
  let totalInserted = 0;
  
  // Generating 40 items per category in 4 batches of 10 items.
  const BATCHES = 4;
  ...
  for (const product of PRODUCT_LINES) {
    ...
    for (let batch = 1; batch <= BATCHES; batch++) {
      console.log(`Generating batch ${batch}/${BATCHES} for ${product.name} (${ITEMS_PER_BATCH} items)...`);
      ...
      console.log(`+ Success: Inserted ${insertedInThisBatch} items from batch ${batch}.`);
      
      // Delay 10 seconds between successful batches to prevent rate limit overloading
      if (batch < BATCHES) {
        console.log(`Taking a 10s cooldown break to prevent API overload...`);
        await sleep(10000);
      }
    }
    console.log(`Finished ${product.name}. Inserted ${categoryInserted} items total.`);
  }
